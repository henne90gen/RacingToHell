...
MenuResult Textbox::handleEvent(sf::Event &Event, sf::Vector2f MousePos) {
    if (Event.type == sf::Event::MouseButtonPressed) {
        ...
        if (_Enabled && _Focused) {
            ...
            for (unsigned long i = _String.length(); i > 0; i--) {
                ...
                if (Event.mouseButton.x >
                    TmpText.getPosition().x + TmpText.getLocalBounds().width - TmpChar.getLocalBounds().width / 2) {
                    _CursorPosition = i;
                    break;
                }
            }
        ...
        if (_Enabled && _Focused) {
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::Return)) {
                ...
                if (Event.key.code == 59 && _Text.getString().getSize() > 0 && _CursorPosition > 0) {
                    setText(_Text.getString().substring(0, _CursorPosition - 1) +
                            _Text.getString().substring(_CursorPosition,
                                                        _Text.getString().getSize() - _CursorPosition));
                    _CursorPosition--;
                    setCursor();
                } else if (Event.key.code == 66 && _Text.getString().getSize() > 0 &&
                           _CursorPosition < _Text.getString().getSize()) {
                    // Delete character with delete
                    setText(_Text.getString().substring(0, _CursorPosition) +
                            _Text.getString().substring(_CursorPosition + 1,
                                                        _Text.getString().getSize() - _CursorPosition));
                }
            }
            ...
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::LControl) && sf::Keyboard::isKeyPressed(sf::Keyboard::V)) {
                std::string newString = _Text.getString().substring(0, _CursorPosition) + GetClipboardText() +
                                        _Text.getString().substring(_CursorPosition,
                                                                    _Text.getString().getSize() - _CursorPosition);
        ...
        if (_Enabled && _Focused) {
            ...
            if ((c > 44 && c < 58) || (c > 64 && c < 91) || (c > 96 && c < 123)) {
                std::string newString = _Text.getString().substring(0, _CursorPosition) + static_cast<char>(c) +
                                        _Text.getString().substring(_CursorPosition,
                                                                    _Text.getString().getSize() - _CursorPosition);
