...
void loadCharacter(GameMemory *memory, FT_Face face, char loadCharacter,
    ...
    if (error) {
        memory->abort("Couldn't load glyph for " +
                      std::to_string(loadCharacter));
    }
    ...
    Render::Character newCharacter =
        gameState->resources
            .characterMap[fontSize][loadCharacter - Render::firstCharacter];
    ...
    newCharacter.size =
        glm::vec2(newCharacter.texture.width, newCharacter.texture.height) *
        scale;
    ...
    if (hasKerning) {
        for (char nextChar = Render::firstCharacter;
            ...
            newCharacter.kerning[nextChar - Render::firstCharacter] =
                (kerning.x >> 6) * scale;
        }
    }

    unsigned bitmapSizeInPixel =
        newCharacter.texture.width * newCharacter.texture.height;
...
void loadFont(GameMemory *memory, std::string fontFileName) {
    ...
    for (unsigned fontSizeIndex = 0;
        ...
        if (error) {
            std::string message =
                "Couldn't set pixel size to " + std::to_string(fontSize) + ".";
