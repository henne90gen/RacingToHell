...
std::shared_ptr<BossCar> GameObjectFactory::getBossCar(GameObjectManager &gom, int level, int diff, int hp) {
    switch (level) {
        case 0: {
            std::shared_ptr<Tank> boss = std::make_shared<Tank>(_DeltaID + _CurrentGameObjectID++, gom, diff, hp,
...
GameObjectFactory::getBullet(GameObjectManager &gom, sf::Vector2f pos, sf::Vector2f dir, int speed,
                             GameObjectType type) {
    sf::IntRect textureRect(0, 0, _BulletTexture().getSize().x, _BulletTexture().getSize().y);
    return std::make_shared<Bullet>(_DeltaID + _CurrentGameObjectID++, gom, pos, dir, speed, type, _BulletTexture());
...
std::shared_ptr<AICar> GameObjectFactory::getAICar(GameObjectManager &gom, int hp, int roadSpeed) {
    ...
    std::shared_ptr<AICar> car(
            new AICar(_DeltaID + _CurrentGameObjectID++, gom, hp, roadSpeed, _AICarTextureMap(), textureRect));
    return car;
}

std::shared_ptr<GameObject> GameObjectFactory::getToolbox(GameObjectManager &gom, sf::Vector2f pos, float speed) {
    ...
    std::shared_ptr<GameObject> toolbox(
            new GameObject(_DeltaID + _CurrentGameObjectID++, gom, GameObjectType::Tools, pos, _ToolboxTexture(),
                           textureRect));
    toolbox->setMovement(sf::Vector2f(0, speed));
    return toolbox;
}

std::shared_ptr<GameObject> GameObjectFactory::getCanister(GameObjectManager &gom, sf::Vector2f pos, float speed) {
    ...
    std::shared_ptr<GameObject> canister(
            new GameObject(_DeltaID + _CurrentGameObjectID++, gom, GameObjectType::Canister, pos,
                           _EnergyCanisterTexture(),
                           textureRect));
...
//std::shared_ptr<BossCar> GameObjectFactory::getBossCar(int level, sf::Packet &packet, PlayerCar &player) {
//    switch (level) {
//        case 0: {
//            std::shared_ptr<Tank> boss(
//                    new Tank(packet, (*_BossCarTextures()[0]), _BulletTexture(), player));
//            return boss;
//        }
//        case 1: {
//            std::shared_ptr<Mech> boss(
//                    new Mech(packet, (*_BossCarTextures()[2]), (*_BossCarTextures()[3]), _BulletTexture(), player));
//            return boss;
//        }
//        case 2: {
//            std::shared_ptr<Jet> boss(
//                    new Jet(packet, (*_BossCarTextures()[4]), _BulletTexture(), _JetSoundBuffer(), player));
//            return boss;
//        }
//        case 3: {
//            std::shared_ptr<Carrier> boss(
//                    new Carrier(packet, (*_BossCarTextures()[1]), _BulletTexture(), player));
//            return boss;
//        }
//        default:
//            return NULL;
...
//            gos.push_back(getBossCar(level, packet, player));
//            break;
//    }
//}
