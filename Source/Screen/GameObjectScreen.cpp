...
GameObjectScreen::GameObjectScreen(Framework &framework) : GameScreen(framework), _PlayerBulletSpeed(600),
                                                           _AIBulletSpeed(400) {}

GameObjectScreen::~GameObjectScreen() {
    _GameObjects.clear();
    _Cars.clear();
    _Bullets.clear();
}
...
void GameObjectScreen::update(/*float FrameTime*/) {
    // FIXME don't use constant value for frame time and road speed
            ...
            if (_GameMode == GameMode::Invincible) {
                std::shared_ptr<Explosion> newExplosion(
                        new Explosion(_Cars.at(i)->getPos(), _ExplosionTexture, sf::Vector2f(0, _Cars[i]->getSpeed()),
                                      _SoundEffects, _ExplosionSoundBuffer, _Volume));
    ...
    for (unsigned int i = 0; i < _Cars.size(); i++) {
        if (_Cars.at(i)->getPos().y - _Cars.at(i)->getHeight() / 2 > SCREENHEIGHT ||
            _Cars.at(i)->getPos().y + _Cars.at(i)->getHeight() / 2 <= 0 ||
            _Cars.at(i)->getPos().x + _Cars.at(i)->getWidth() / 2 <= 0 ||
            _Cars.at(i)->getPos().x - _Cars.at(i)->getWidth() / 2 >= SCREENWIDTH) {
            deleteObject(_Cars, i);
            i--;
        }
    }
    ...
    for (unsigned int i = 0; i < _Bullets.size(); i++) {
        if (_Bullets.at(i)->getPos().y - _Bullets.at(i)->getHeight() / 2 > SCREENHEIGHT ||
            _Bullets.at(i)->getPos().y + _Bullets.at(i)->getHeight() / 2 <= 0 ||
            _Bullets.at(i)->getPos().x + _Bullets.at(i)->getWidth() / 2 <= 0 ||
            _Bullets.at(i)->getPos().x - _Bullets.at(i)->getWidth() / 2 >= SCREENWIDTH) {
            deleteObject(_Bullets, i);
            i--;
        }
    }
    ...
    for (unsigned int i = 0; i < _GameObjects.size(); i++) {
        if (_GameObjects.at(i)->getPos().y - _GameObjects.at(i)->getHeight() / 2 > SCREENHEIGHT ||
            _GameObjects.at(i)->getPos().y + _GameObjects.at(i)->getHeight() / 2 <= 0 ||
            _GameObjects.at(i)->getPos().x + _GameObjects.at(i)->getWidth() / 2 <= 0 ||
            _GameObjects.at(i)->getPos().x - _GameObjects.at(i)->getWidth() / 2 >= SCREENWIDTH) {
            deleteObject(_GameObjects, i);
            i--;
        }
    }
    ...
    if ((bulletDir.x != 0 || bulletDir.y != 0) && (_GameMode == GameMode::InfEnergy || _Player->drainShotEnergy())) {
        _Bullets.push_back(GameObjectFactory::getBullet(_Player->getPos(), bulletDir, _PlayerBulletSpeed,
                                                        GameObjectType::BulletObjectPlayer, _SoundEffects, _Volume));
    }
}

void GameObjectScreen::render(sf::RenderWindow &window) {
    // FIXME don't use constant for renderCrosshair
    bool renderCrosshair = true;
...
void GameObjectScreen::playSounds() {
...
void GameObjectScreen::stopSounds() {
    _SoundEffects.clear();
    if (_BossFight) {
...
bool GameObjectScreen::bossIsDead() {
    if (_BossFight) {
...
void GameObjectScreen::enterBossFight() {
    _Boss = GameObjectFactory::getBossCar((_Level - 1) % 4, _Difficulty, getBossHP(), _SoundEffects,
                                          _ExplosionSoundBuffer, _Volume);
    _BossFight = true;
}

void GameObjectScreen::resetGameObjects(int SelectedCar) {
    _GameObjects.clear();
    _Cars.clear();
    _Bullets.clear();
    _Animations.clear();
...
bool GameObjectScreen::emptyScreen() {
    _AboutToLevelUp = true;
    if (_Cars.size() == 0) {
        _AboutToLevelUp = false;
        return true;
...
void GameObjectScreen::setCarSkins(std::vector<std::shared_ptr<sf::Texture>> &CarSkins) {
    GameObjectFactory::setPlayerCarTextures(CarSkins);
}

// TODO decide what is going to happen with this method
/*
	void GameObjectScreen::playShotSound(GameObjectType go, sf::Vector2f position)
	{
...
void GameObjectScreen::spawnAICar(int roadSpeed) {
...
void GameObjectScreen::spawnBullet() {
    if (_Cars.size() == 0)
        return;
...
bool GameObjectScreen::playerIsAlive() {
...
void GameObjectScreen::setAllFrequencies() {
    setAiCarFrequency();
    setBulletFrequency();
...
void GameObjectScreen::setAiCarFrequency() {
    switch (_Difficulty) {
        case 0:
...
void GameObjectScreen::setBulletFrequency() {
    switch (_Difficulty) {
        case 0:
...
void GameObjectScreen::setCanisterFrequency() {
    switch (_Difficulty) {
        case 0:
...
void GameObjectScreen::setToolboxFrequency() {
    switch (_Difficulty) {
        case 0:
...
int GameObjectScreen::getBossHP() {
    switch ((_Level - 1) % 4) {
        case 0:
...
void GameObjectScreen::playHitSound(sf::Vector2f position) {
