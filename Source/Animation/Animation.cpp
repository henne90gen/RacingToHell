...
Animation::Animation(sf::Vector2f pos, sf::Texture &spriteSheet, float timePerFrame, int frames, int rows, int cols) :
        _AnimationState(AnimationState::Play), _TimePerFrame(timePerFrame), _NumFrames(frames), _NumRows(rows),
        _NumCols(cols) {
    _SpriteSheet = spriteSheet;
    _Sprite.setTexture(_SpriteSheet);
    _CurrentSpriteRect = sf::IntRect(0, 0, (int) (_Sprite.getLocalBounds().width / _NumCols),
                                     (int) (_Sprite.getLocalBounds().height / _NumRows));
...
int Animation::getCurrentFrameNum() {
    return _CurrentSpriteRect.top / _CurrentSpriteRect.height * _NumCols +
