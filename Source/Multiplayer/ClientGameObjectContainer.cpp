...
ClientGameObjectContainer::ClientGameObjectContainer() : _PlayerBulletSpeed(600), _AIBulletSpeed(400) {}

ClientGameObjectContainer::~ClientGameObjectContainer()
{
	_GameObjects.clear();
}

void ClientGameObjectContainer::update(float FrameTime, int RoadSpeed)
{
	// Update Animations
...
void ClientGameObjectContainer::render(sf::RenderWindow& Window, bool renderCrosshair)
{
...
void ClientGameObjectContainer::handleEvent(sf::Event& newEvent)
{
	for (unsigned int i = 0; i < _GameObjects.size(); i++)
	{
		if (_PlayerAlive) {
...
void ClientGameObjectContainer::playSounds()
{
...
void ClientGameObjectContainer::stopSounds()
{
	_SoundEffects.clear();
	if (_BossFight) {
...
bool ClientGameObjectContainer::bossIsDead()
{
	if (_BossFight) {
...
void ClientGameObjectContainer::enterBossFight()
{
...
void ClientGameObjectContainer::resetGameObjects(int SelectedCar)
{
	_GameObjects.clear();
	_Animations.clear();
	_SoundEffects.clear();
...
bool ClientGameObjectContainer::emptyScreen()
{
	_AboutToLevelUp = true;
...
void ClientGameObjectContainer::load()
{
...
void ClientGameObjectContainer::setCarSkins(std::vector<std::shared_ptr<sf::Texture>>& CarSkins)
{
...
void ClientGameObjectContainer::spawnAICar(int roadSpeed)
{
...
void ClientGameObjectContainer::spawnBullet()
{
...
bool ClientGameObjectContainer::playerIsAlive() {
...
void ClientGameObjectContainer::deleteObject(unsigned int id)
{
	_GameObjects.erase(_GameObjects.begin() + id);
}

void ClientGameObjectContainer::setAllFrequencies()
{
	setAiCarFrequency();
	setBulletFrequency();
...
void ClientGameObjectContainer::setAiCarFrequency()
{
	switch (_Difficulty)
	{
	case 0:
...
void ClientGameObjectContainer::setBulletFrequency()
{
	switch (_Difficulty)
	{
	case 0:
...
void ClientGameObjectContainer::setCanisterFrequency()
{
	switch (_Difficulty)
	{
	case 0:
...
void ClientGameObjectContainer::setToolboxFrequency()
{
	switch (_Difficulty)
	{
	case 0:
...
int ClientGameObjectContainer::getAiHP()
{
	switch (_Difficulty)
	{
	case 0:
...
int ClientGameObjectContainer::getBossHP()
{
	switch ((_Level - 1) % 4)
	{
	case 0:
...
void ClientGameObjectContainer::playHitSound(sf::Vector2f position)
{
